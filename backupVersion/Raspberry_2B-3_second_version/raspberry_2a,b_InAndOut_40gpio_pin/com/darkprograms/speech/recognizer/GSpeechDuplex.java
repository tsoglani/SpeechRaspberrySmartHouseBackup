        ...
        new Thread("Downstream Thread") {
            public void run() {
                ...
                // httpsUrlConn and allow it enough time to do its work.
                 inStream = openHttpsConnection(url);
                if (inStream == null) {
                    //ERROR HAS OCCURED
    ...
    private void openHttpsPostConnection(String murl, TargetDataLine mtl, int sampleRate) {
        ...
        try {
            ...
            if (!(urlConn instanceof HttpsURLConnection)) {
             ...
             httpConn2 = (HttpsURLConnection) urlConn;
            httpConn2.setAllowUserInteraction(false);
            httpConn2.setInstanceFollowRedirects(true);
            httpConn2.setRequestMethod("POST");
            httpConn2.setDoOutput(true);
            httpConn2.setChunkedStreamingMode(0);
            httpConn2.setRequestProperty("Transfer-Encoding", "chunked");
            httpConn2.setRequestProperty("Content-Type", "audio/x-flac; rate=" + sampleRate);
            // also worked with ("Content-Type", "audio/amr; rate=8000");
            httpConn2.connect();

            // this opens a connection, then sends POST & headers.
            final OutputStream out = httpConn2.getOutputStream();
            ...
            //Diagonostic Code.
            /*int resCode = httpConn2.getResponseCode();
			if (resCode / 100 != 2) {
				System.out.println("ERROR");
			}
			Scanner scanner = new Scanner(httpConn2.getInputStream());
			while(scanner.hasNextLine()&&destroy){
            ...
            ex.printStackTrace();
            openHttpsPostConnection(murl, mtl, sampleRate);
        }
    }
